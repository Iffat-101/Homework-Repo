...
lim = 1e3;      % limit for loops to run;
...
%% Initialize the Flow, Inhomogenous IC
 ...
 lim = lim; % 1e10; % iteration stopper, check
...
%     [U_now_u_vw, U_now_v_vw] = MatView(U_now_u, U_now_v);
    ...
    % for u's
    chtit1 = 'Countour, $$ u_1 $$';
    chrx1 = 'Domain length, $$ x_1(m) $$'; chry1 = 'Domain Width, $$ x_2(m) $$';
    [Vw_u] = PrintMat(Mat_u, 11,  chtit1, chrx1, chry1);
    
    % for v's
    chtit2 = 'Countour, $$ v_1 $$';
    chrx2 = 'Domain length, $$ x_1(m) $$'; chry2 = 'Domain Width, $$ x_2(m) $$';  % for v's
    [Vw_v] = PrintMat(Mat_v, 12,  chtit2, chrx2, chry2); 
    
    % for p's
    chtitP = 'Countour, $$ P $$';
    chrxP = 'Domain length, $$ x_1(m) $$'; chryP = 'Domain Width, $$ x_2(m) $$';
    [Vw_p] = PrintMat(Mat_p, 13,  chtitP, chrxP, chryP);

    % for Grd-p's
    chtitPG = 'Countour, $$ \nabla P $$';
    chrxPG = 'Domain length, $$ x_1(m) $$'; chryPG = 'Domain Width, $$ x_2(m) $$';
